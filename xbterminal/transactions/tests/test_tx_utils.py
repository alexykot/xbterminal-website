...
class TxUtilsTestCase(TestCase):
    ...
    def test_create_tx_dust_output(self):
        tx_inputs = [self.get_tx_input()]
        tx_outputs = {'n2p8YJojY7zewwt4ngonYUPgkV1xy7qfjR': 0}
        with self.assertRaises(DustOutput):
            create_tx(tx_inputs, tx_outputs)

    def test_create_tx_bad_signature(self):
        tx_inputs = [self.get_tx_input(private_key=BIP32Node.from_hwif(
